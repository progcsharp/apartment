...
async def get_all(db=Depends(get_db)):
    reservation = await get_reservation_all(db)
    return reservation
...
@router.get("/id/{id}", response_model=ReservationResponse)
async def get_by_id(id: int, db=Depends(get_db)):
    reservation = await get_reservation_by_id(id, db)
    return reservation


@router.get("/clientid/{client_id}", response_model=List[ReservationResponse])
async def get_by_user_id(client_id: int, db=Depends(get_db)):
    reservation = await get_reservation_by_client_id(client_id, db)
    return reservation
...
async def update(reservation_data: ReservationUpdate, db=Depends(get_db)):
    reservation = await update_reservation(reservation_data, db)
    return reservation
...
async def delete(reservation_id: int, db=Depends(get_db)):
    reservation = await delete_reservation(reservation_id, db)
    return reservation
